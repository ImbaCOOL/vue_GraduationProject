  ...
  <div>
    <el-card class="card">
      ...
      <el-form ref="form" :model="form" label-width="auto" @submit.native.prevent status-icon inline>
        ...
        <el-form-item label="入学日期：" prop="date"
          :rules="[
            { required: true, message: '请选择', trigger: 'blur' }
        ]">
          <el-date-picker
...
export default {
  data() {
    return {
      form: {
        username: sessionStorage.getItem("student").split(",")[0],
        stuID: '',
        name:'',
        cellphone: '',
        gender:'',
  ...
  async created(){
    let {err,data}=await loginAPI.loginByStu(sessionStorage.getItem("student").split(",")[0],sessionStorage.getItem("student").split(",")[1])
    if(err===0){
      this.form.stuID=data.stuID
